...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Product[] products = {new Meat(), new Cheese(), new Juice(), new Tablet()};
        //принцип DRY - повторяющийся вывод списка продуктов на экран выносим в отдельный метод
        printProucts(products);
        Scanner scanner = new Scanner(System.in);
        while (true) {
            System.out.println("Введите номер товара и  его количество или введите `escape` ");
            String input = scanner.nextLine();
            if ("escape".equals(input)) break;
